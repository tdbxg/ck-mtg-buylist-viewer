...
function renderRows() {
  ...
  $("rows").innerHTML = rows.map((p) => {
    ...
    return `<tr>
      ...
      <td><span class="badge ${riskClass}">${esc(p.risk_level || "正常")}</span>${p.below_ck_buylist ? '<span class="badge opportunity">低于 CK 回收价</span>' : ""}${p.deterministic_fix ? `<small class="fixed">${esc(p.deterministic_fix)}</small>` : ""}${p.external_link_issue ? `<small class="bad">外链：${esc(p.external_link_issue)}</small>` : ""}${(p.risk_reasons || []).filter((r) => r !== p.external_link_issue).slice(0, 2).map((r) => `<small>${esc(r)}</small>`).join("")}</td>
