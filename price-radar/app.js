...
function applyFilters(resetPage = true) {
  ...
  state.filtered = state.products.filter((p) => {
    const haystack = [p.product_name, p.sku, p.product_id, p.series, p.category, p.ck_name].join(" ").toLowerCase();
  ...
  if (sort === "price-desc") state.filtered.sort(numeric("sale_price"));
  if (sort === "price-asc") state.filtered.sort((a, b) => (Number(a.sale_price) || Infinity) - (Number(b.sale_price) || Infinity));
...
function renderRows() {
  ...
  $("rows").innerHTML = rows.map((p) => {
    ...
    return `<tr>
      <td><strong>${esc(p.product_name || p.sku || "未命名商品")}</strong><small>${esc(p.sku || "")}</small><small>ID ${esc(p.product_id || "—")}</small></td>
      <td><strong>${esc(p.category || "—")}</strong><small>${esc(p.series || "")}</small></td>
      <td class="money">${esc(p.stock ?? "—")}</td>
      <td class="money">${money(p.cost)}</td>
      <td class="money">${money(p.sale_price)}${p.external_market_price ? `<small>外网 ${money(p.external_market_price)}</small>` : ""}</td>
...
async function init() {
  try {
    ...
    $("productCount").textContent = Number(meta.product_count || 0).toLocaleString("zh-CN");
    $("ckExact").textContent = Number(meta.ck_exact_match_count || 0).toLocaleString("zh-CN");
    $("ckBelow").textContent = Number(meta.ck_below_buylist_count || 0).toLocaleString("zh-CN");
    $("linkIssues").textContent = Number(meta.external_link_issue_count || 0).toLocaleString("zh-CN");
    [...new Set(products.map((p) => p.category).filter(Boolean))].sort().forEach((value) => $("category").insertAdjacentHTML("beforeend", `<option value="${esc(value)}">${esc(value)}</option>`));
    applyFilters();
  } catch (error) {
